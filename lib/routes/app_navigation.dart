...
class AppNavigator {
  static Future toUserPage() async {
    return await Get.toNamed(RoutePath.kUser);
  }

  static Future toVideoPlayer(VideoItemModel item) async {
    return await Get.toNamed(RoutePath.kVideoPlayer, arguments: item);
  }

  /// 跳转至博客详情
  // static Future toBlogContent({required String url}) async {
  //   return await Get.toNamed(RoutePath.kBlogContent, parameters: {"url": url});
  // }
  ...
  static Future toBlogComment({
    ...
    //   parameters: {"blogApp": blogApp, "postId": postId.toString()},
    // );
  }
  //
  // /// 跳转至知识库详情
  ...
  //     blogApp =
  //         RegExp(r"cnblogs.com/(.*?)/").firstMatch(blogApp)?.group(1) ??
  //         blogApp;
  //   }
  //   return await Get.toNamed(RoutePath.kUserBlog, arguments: blogApp);
  // }
  ...
  static Future toWebView(String url) async {
    ...
    final RegExp blogRegExp2 = RegExp(
      r'cnblogs.com/(.*?)/archive/\d+/\d+/\d+/(.*?).html',
    );
    ...
    return await Get.toNamed(RoutePath.kWebView, arguments: url);
  }

  // /// 获取闪存详情
  // static Future toStatusesDetail(int id) async {
  //   return await Get.toNamed(RoutePath.kStatusesDetail, arguments: id);
  // }
  //
  // /// 获取问题详情
  // static Future toQuestionDetail(int id) async {
  //   return await Get.toNamed(RoutePath.kQuestionDetail, arguments: id);
  // }
  //
  // /// 跳转至回答评论
  ...
  //
  // /// 跳转至博文评论
  // static Future toNewsComment({required int newsId}) async {
  //   return await Get.toNamed(
